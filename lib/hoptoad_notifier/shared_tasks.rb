namespace :hoptoad do
  ...
  namespace :heroku do
    ...
    task :add_deploy_notification => [:environment] do
      ...
      command = %Q(heroku addons:add deployhooks:http url="http://hoptoadapp.com/deploys.txt?deploy[local_username]={{user}}&deploy[scm_revision]={{head_long}}&deploy[rails_env]=#{heroku_rails_env}&api_key=#{heroku_api_key}")
